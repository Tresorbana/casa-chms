      ...
      <div className="flex gap-1 p-1 bg-muted border border-border rounded-lg w-fit">
        {(['overview', 'expenses', 'income'] as const).map(tab => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-xs font-medium transition-all capitalize ${activeTab === tab ? 'bg-card text-foreground shadow-sm border border-border' : 'text-muted-foreground hover:text-foreground'}`}
          >
            {tab === 'income' ? 'Income Breakdown' : tab.charAt(0).toUpperCase() + tab.slice(1)}
          </button>
        ))}
      </div>
