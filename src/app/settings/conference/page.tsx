        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 lg:p-8 flex flex-col gap-8">
            ...
            {isAdding && (
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-lg border border-primary/20 animate-in fade-in slide-in-from-top-4">
                    <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4">{editingRoom ? 'Edit Conference Room' : 'New Conference Room'}</h3>
                    ...
                    <div className="flex gap-4">
                        ...
                        <button onClick={() => { setIsAdding(false); setEditingRoom(null); }} className="bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-6 py-2 rounded-xl font-bold">Cancel</button>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {rooms?.map((room: any) => (
                    <div key={room.id} className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-xl transition-all group">
                        ...
                        <h3 className="text-xl font-black text-slate-800 dark:text-white mb-2">{room.name}</h3>
                        <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                                <span className="text-slate-500 font-bold">Capacity</span>
                                <span className="font-bold text-slate-800 dark:text-slate-200">{room.capacity} Pax</span>
                            </div>
                            <div className="flex justify-between text-sm">
                            ...
                            {room.bookings && room.bookings.length > 0 ? (
                                <ul className="space-y-2">
                                    {room.bookings.slice(0, 2).map((b: any) => (
                                        <li key={b.id} className="text-xs text-slate-600 dark:text-slate-400 flex justify-between">
