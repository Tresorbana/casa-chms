        ...
        <div className="min-h-screen bg-slate-50 p-4 lg:p-8 flex flex-col gap-8">
            ...
            {isAddingFloor && (
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-primary/20 animate-in fade-in slide-in-from-top-4">
                    <h3 className="text-lg font-bold text-slate-800 mb-4">New Floor</h3>
                    <div className="flex gap-4">
                        <input
                            type="number"
                            placeholder="Floor Number"
                            className="flex-1 bg-slate-50 border-none rounded-xl px-4 py-2 outline-none focus:ring-2 focus:ring-primary"
                            value={newFloorNumber}
                            onChange={e => setNewFloorNumber(e.target.value)}
                        />
                        <button onClick={handleAddFloor} className="bg-primary text-white px-6 py-2 rounded-xl font-bold">Save</button>
                        <button onClick={() => setIsAddingFloor(false)} className="bg-slate-200 text-slate-600 px-6 py-2 rounded-xl font-bold">Cancel</button>
                    </div>
                </div>
            )}
            ...
            <div className="grid gap-6">
                {floors?.map((floor: any) => (
                    <div key={floor.id} className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
                        <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-xl font-bold text-slate-800">Floor {floor.number}</h3>
                        ...
                        {addingRoomToFloor === floor.id && (
                            <div className="mb-6 p-4 bg-slate-50 rounded-2xl flex flex-wrap gap-4 items-end animate-in fade-in">
                                <div className="flex-1 min-w-[120px]">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1 block">Number</label>
                                    <input
                                        className="w-full bg-white rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary"
                                        placeholder="e.g. 101"
                                ...
                                <button onClick={() => setAddingRoomToFloor(null)} className="text-slate-400 px-4 py-2 rounded-lg text-sm font-bold hover:bg-slate-200">Cancel</button>
                            </div>
                        )}

                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            {floor.rooms?.map((room: any) => (
                                <div key={room.id} className="relative group bg-slate-50 p-4 rounded-2xl border border-slate-100 hover:border-primary/50 transition-all">
                                    ...
                                    <div className="text-center">
                                        ...
                                        <span className="text-lg font-black text-slate-800 block">{room.number}</span>
                                        <span className="text-sm font-bold text-primary block mt-1">RWF {room.price?.toLocaleString()}</span>
                                    </div>
                                </div>
                            ))}
