...
export default function Dashboard() {
  ...
  const { data, error, isLoading, mutate } = useSWR(`/api/dashboard?date=${selectedDate}`, fetcher);
  const [selectedRoom, setSelectedRoom] = useState<any>(null);

  if (isLoading || !data) return <div className="p-8 text-center text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Loading System Data...</div>;
  if (error) return <div className="p-8 text-center text-red-400 font-black">Connection Failure</div>;
  ...
  return (
    <div className="min-h-screen p-4 lg:p-8 bg-slate-50 flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {/* Stat Cards */}
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm transition-all">
      ...
      <section className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Live hotel room status for {new Date(selectedDate).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</p>
          </div>
              ...
              <div key={floorNum}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                  {roomsByFloor[floorNum].map((room: any) => (
                    ...
                    >
                      <div className={`absolute top-4 right-4 w-2.5 h-2.5 rounded-full ${room.displayStatus === 'OCCUPIED' ? 'bg-primary' : 'bg-secondary'} animate-pulse`}></div>
