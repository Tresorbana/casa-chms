  ...
  return (
    <div className="flex-1 min-h-screen p-4 lg:p-8 flex flex-col gap-8 bg-slate-50 dark:bg-slate-950">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {venues.map((venue, i) => (
          <div key={i} className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-2xl group hover:scale-[1.02] transition-all">
            ...
            <div className="p-8">
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                <div>
                  ...
                  <p className="text-sm font-black text-center text-slate-700 dark:text-slate-200 uppercase tracking-tighter">{venue.rate}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1 text-center">Capacity</p>
                  <p className="text-sm font-black text-center text-slate-700 dark:text-slate-200 uppercase tracking-tighter">{venue.capacity} GUESTS</p>
                </div>
              </div>
              <button className="w-full mt-8 py-4 bg-slate-50 dark:bg-slate-800/50 hover:bg-primary hover:text-white transition-all rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-100 dark:border-slate-800 flex items-center justify-center gap-2 group-hover:border-primary/20">
                <span className="material-icons-outlined text-sm">analytics</span> View Logistics
              </button>
            </div>
          </div>
        ))}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-2xl flex-1">
        ...
        <div className="p-8">
          <div className="space-y-6 relative border-l-2 border-slate-100 dark:border-slate-800 pl-8 ml-4">
            {schedule.map((ev, i) => (
              <div key={i} className="relative group grayscale hover:grayscale-0 transition-all">
                <div className={`absolute -left-[41px] top-1 w-4 h-4 rounded-full ${ev.color === 'primary' ? 'bg-olive-leaf' : ev.color === 'secondary' ? 'bg-copper' : 'bg-blue-500'} ring-4 ring-white dark:ring-slate-900 shadow-sm`}></div>
                <div className="bg-slate-50 dark:bg-slate-800/20 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 group-hover:border-primary/30 group-hover:shadow-lg transition-all">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-black text-lg italic tracking-tighter uppercase text-slate-800 dark:text-white">{ev.title}</h4>
